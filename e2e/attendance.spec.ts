...
test.describe("Attendance", () => {
  ...
  test("attendance report button visible", async ({ page }) => {
    await openAttendance(page);
    await expect(
      page.getByRole("button", { name: /Attendance Report/i })
    ).toBeVisible();
  });
  ...
  test("start attendance session", async ({ page }) => {
    ...
    const noStudents = page.getByText("No students found for this batch.");
    const saveButton = page.getByRole("button", { name: /Save Attendance/i });
    await expect(noStudents.or(saveButton)).toBeVisible();
  });
  ...
  test("batch selector change", async ({ page }) => {
    ...
    if (optionCount > 1) {
      const value = await batch.locator("option").nth(1).getAttribute("value");
      if (value) {
