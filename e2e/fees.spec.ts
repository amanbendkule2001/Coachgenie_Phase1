...
test.describe("Fees", () => {
  ...
  test("search input visible", async ({ page }) => {
    await expect(
      page.locator('input[placeholder="Search student, invoice…"]')
    ).toBeVisible();
  });
  ...
  test("refresh button works", async ({ page }) => {
    const refresh = page.getByRole("button", { name: "" }).first();
    await refresh.click();
    await expect(
      page.getByRole("heading", { name: /Fee Management/i })
    ).toBeVisible();
  ...
  test("search accepts text", async ({ page }) => {
    const input = page.locator('input[placeholder="Search student, invoice…"]');
  ...
  test("status filters clickable", async ({ page }) => {
    const filters = ["Paid", "Pending", "Partial", "Overdue"];
  ...
  test("table headers visible", async ({ page }) => {
    ...
    await expect(table.getByRole("button", { name: "Invoice #" })).toBeVisible();
    await expect(table.getByRole("button", { name: "Student" })).toBeVisible();
    await expect(table.getByRole("button", { name: "Amount" })).toBeVisible();
    await expect(table.getByRole("button", { name: "Status" })).toBeVisible();
    await expect(table.getByRole("button", { name: "Due Date" })).toBeVisible();
  });
